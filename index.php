...
if ($pages == 'tour-page') {
  ...
  // get data
  $sql = "SELECT * FROM `armenia` WHERE `id`='$armeniaId'";
  $result = mysqli_query($conn, $sql);
  if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while ($row = mysqli_fetch_assoc($result)) {
  ...
  $sql = "SELECT * FROM `pages` WHERE `name`='$pages'";
  $result = mysqli_query($conn, $sql);
  if (mysqli_num_rows($result) > 0) {
    // output data of each row
    while ($row = mysqli_fetch_assoc($result)) {
...
<section style="height: <?= $bg ?> ;
  ...
  <div class="<?php echo $pages ?> container text-center">
    <div style="top: <?= $headerPos ?>" class="<?php echo $pages ?> main-text-box">
      ...
      <p class="<?php echo $pages ?> h6 text-white"><?php echo $paragraph ?></p>
    </div>
  </div>
